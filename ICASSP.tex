...
Epochs & \multicolumn{3}{l|}{Up to 200 (early stop patience=30)} \\ \hline
...
\caption{Comparison of RIR estimation performance across baseline and proposed methods on L-shaped and irregular target rooms. Methods compared: GAN-based RIR synthesis \cite{Ratnarajah2022GAN_RIR}, low-rank estimation \cite{Jalmby2023TASLP}, source-domain pretraining only, and the proposed transfer learning approach. Results are averaged over 3 random seeds and 20 test room configurations.}
...
\caption{Comparison of three fine-tuning strategies on target domain: no fine-tuning (source-only baseline), fine-tuning only the output layer, and the proposed strategy which freezes the encoder while adapting the decoder. MSE is shown in units of $\times 10^{-3}$.}
